...
The first things we need to do is use our metadata file (currently defined by the environmental variable `METADATA`) to make three individual files containing just the list of individuals in each of the populations. We can do this by subsetting our sample metadata file, using the command `grep` to grab lines that match each population's name, and then using `awk` to keep only the first column of metadata, i.e. the sample names.
